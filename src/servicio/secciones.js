import yo from 'yo-yo'
module.exports = [
  {
    titulo: 'ACONDICIONAMIENTO ACÚSTICO',
    icono: yo`<span class="fa icon-sonometro"></span>`,
    posicion: '400',
    imagen: 'https://images.pexels.com/photos/337542/pexels-photo-337542.jpeg',
    ilustracion: 'imagenes/servicios-acondicionamiento-acustico.png',
    descripcion:'Ajuste, control y optimiazción de teatros, estudios de grabación, bares, restaurantes, salones de conferencias, etc.',
    explicacion: 'Uno de los mayores problemas del audio es la sala dónde se reproduce el mismo. Ofrecemos a cada cliente un diseño personalizado para bares, discotecas, restaurantes, salas de conferencias, casas particulares, teatros, etc… ; con lo que logramos optimizar las características de la sala y la calidad del sonido. Para ello utilizamos las últimas técnicas en simulación que nos permiten valorar previamente la distribución del sonido en la sala.',
    contenido: yo`
    <div>
      <p>Los recintos destinados a servicios acústicos requieren de una acústica interior casi perfecta. Una gran variedad de materiales y elementos acústicos junto con las técnicas acústicas más innovadoras en simulación de espacios acústicos, serán dispuestos para lograr la mejora de la calidad del sonido en el interior de diferentes recintos tales como teatros, salones de conferencia, estudios de grabación, emisoras de radio, salas multiuso,etc.</p>
      <p>Si requiere asesoría o diseño de un espacio dedicado a la acústica, EA Panamá se pondrá a su servicio para lograr la optimización de las características de la sala y la calidad del sonido.</p>
      <p>Se ofrece:</p>
      <ul>
        <li>Acústica Arquitectónica: Diseño interno del espacio</li>
        <li>Consultoría acústica de los espacios sonoros y materiales a utilizar</li>
        <li>Modelación acústica por software para optimizar el espacio</li>
        <li>Elaboración de planos constructivos y conceptuales en el tema acústico</li>
        <li>Planos detalle de elementos acústicos del proyecto</li>
      </ul>
    </div>
    `
  },
  {
    titulo: 'AISLAMIENTO ACÚSTICO',
    icono: yo`<i class="fa fa-headphones" aria-hidden="true"></i>`,
    posicion: '800',
    imagen: 'https://images.pexels.com/photos/268362/pexels-photo-268362.jpeg',
    ilustracion: 'imagenes/servicios-aislamiento-acustico.png',
    descripcion:'Diseño y análisis de estructuras de techo, paredes y suelos para la mejora del aislamiento del ruido en su hogar, oficina, restaurante, salón, teatro, etc.',
    explicacion:'Convivimos diariamente con focos de ruido y vibraciones que nos impiden realizar nuestra actividad cotidiana. En EA Panamá somos conscientes de este problema y ofrecemos diseños de sistemas de aislamiento acústico exclusivos para cada caso, con objeto de evitar interferencias entre recintos colindantes y de esta forma evitar la trasmisión no deseada de ruidos y vibraciones.',
    contenido: yo`
    <div>
      <p>Convivimos diariamente con focos de ruido y vibraciones que nos impiden realizar nuestra actividad cotidiana. En EA Panamá somos conscientes de este problema y ofrecemos diseños y servicios de sistemas de aislamiento acústico exclusivos para cada caso.</p>
      <p>Como expertos en esta área, realizamos el asesoramiento acústico en proyectos de edificación, obra nueva o rehabilitación a profesionales del sector como arquitectos, arquitectos técnicos e ingenieros. Además llevamos a cabo el suministro de los materiales más idóneos para evitar transmisión de vibraciones y aislamientos.</p>
      <p>Las acciones correctoras a realizar para atenuación de niveles de ruido son las siguientes:</p>
      <ul>
        <li>Acondicionamiento acústico.</li>
        <li>Aislamiento acústico.</li>
        <li>Cerramientos, cabinas y barreras acústicas.</li>
        <li>Puertas y visores acústicos.</li>
        <li>Tratamiento de entradas/salidas de aire, chimeneas, etc.</li>
        <li>Control de la vibración estructural.</li>
      </ul>
    </div>
    `
  },
  {
    titulo: 'CONTROL DE RUIDO INDUSTRIAL',
    icono: yo`<i class="fa fa-industry" aria-hidden="true"></i>`,
    posicion: '1200',
    imagen: 'https://images.pexels.com/photos/208714/pexels-photo-208714.jpeg',
    ilustracion: 'imagenes/servicios-ruido-industrial.png',
    descripcion:'Estudios y propuestas especializadas para el control de ruido en industria, generadores eléctricos, diseño de filtros, barreras de sonido, sistemas HVAC, maquinaria, etc.',
    explicacion:'La salud auditiva en el entorno de trabajo es de vital importancia para una buena higiene laboral. EA Panamá elabora informes de medidas del ruido industrial además de ofrecer planes de acciones contra los ruidos generados por las diversas maquinarias dentro de las fábricas. De este modo, la industria puede seguir con los más altos estándares de calidad en higiene laboral estipulados por normativas internacionales así como la Organización Mundial de la Salud (OMS).',
    contenido: yo`
    <div>
      <p>En situaciones, como por ejemplo el ruido industrial, hay varias fuentes de sonido, varios caminos para el sonido, y más de un receptor, pero los principios fundamentales del control de ruido en estas situaciones sería el mismo que para el de casos simples.</p>
      <p>El objetivo de los programas de control de ruido es reducir el nivel de sonido indeseado en el receptor. Este puede llevarse a cabo haciendo cambios a la fuente, el camino, o el receptor, o de cualquier combinación de estos elementos.</p>
      <p>Este tipo de situaciones requiere un estudio exhaustivo de todos los componentes de la maquinaria ruidosa, de modo que al conocer cada fuente individualmente podremos valorar su contribución al ruido total (aéreo + vibración).</p>
      <p>El camino, para el sonido que viaja de fuente a receptor, puede ser el aire como es el caso del ruido de las máquinas que transmiten directamente a los oídos del operador. Pero también podría ser el caso de propagación indirecta, cuando el sonido se refleja en una pared de la habitación y se recibe en los oídos del operador.</p>
      <p>También sobre superficies sólidas, tales como tuberías entre una bomba vibratoria y aquellos elementos de la máquina que sirven como ruta para que el ruido se propague por las estructuras.</p>
      <p>Por ello, es importante que un Ingeniero Acústico valore todos los posibles caminos de propagación, considerando su contribución al ruido total, dado que de esta manera se puede tener la visión total del problema de ruido para dar una solución óptima al problema de ruido.</p>
    </div>
    `
  },
  {
    titulo: 'INTEGRACIÓN DE AUDIO Y VIDEO',
    icono: yo`<i class="fa fa-play" aria-hidden="true"></i>`,
    posicion: '1600',
    imagen: 'https://images.pexels.com/photos/207691/pexels-photo-207691.jpeg',
    ilustracion: 'imagenes/servicios-integracion.png',
    descripcion:'Estudio de la ubicación de los parlantes, cantidad y características técnicas tanto de los equipos como de la sala, para obtener la perfección del sistema electroacústico.',
    explicacion:'Dedicamos una división de la empresa de manera exclusiva para la instalación de equipos de audio y vídeo tanto a particulares como a empresas. Nuestros expertos valorarán el recinto así como los equipos necesarios para obtener el mayor rendimiento de la instalación y la sala.',
    contenido: yo`
    <div>
      <p>Realizamos todo tipo de instalaciones de sistemas de sonido, ya sea para particulares o empresa, con los productos de las mejores marcas comerciales, Lynx, Ashly, Bose o Yamaha entre otras.</p>
      <p>EA Panamá va más allá de la simple instalación de equipos audiovisuales en un local o vivienda ofreciendo la mayor funcionalidad posible. Control inalámbrico de la selección musical, distintas zonas con distinta música con control desde tu celular o tablet, acabados bajo selección del cliente, entre muchas otras opciones.</p>
      <p>Nuestros expertos valorarán el recinto así como los equipos necesarios para obtener el mayor rendimiento del sistema de sonido instalado y a la sala de escucha.</p>
    </div>
    `
  }
]
